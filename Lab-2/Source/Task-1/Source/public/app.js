...
app.controller('loginCtrl', function ($scope, $http, $window)
{
    $scope.login = function ()
    {
        ...
        if (userid != "" && userpass != "")
        {
            ...
            $http({url:"http://localhost:8080/login/u=" + userid + "&p=" + userpass , method: 'POST'}).then(function(data, status){
...
app.controller('registerController',function($scope,$http,$window){
    $scope.register = function() {
        ...
        if (userid != "" && userpass != "")
        {
            ...
            $http({url:"http://localhost:8080/register/u=" + userid + "&p=" + userpass, method: 'POST'}).then(function(data, status){
...
app.controller('homeCtrl',function($scope,$http){
    $scope.update = function() {
        ...
        if (color != "" && height != "" && weight != "" && user != "") {
            ...
            $http({url: "http://localhost:8080/update/u=" + user + "&c=" + color + "&h=" + height + "&w=" + weight, method: 'POST'}).then(function (data, status) {
    ...
    $scope.delete = function() {
        ...
        if (user != "") {
            ...
            $http({url: "http://localhost:8080/delete/u=" + user, method: 'POST'}).then(function (data, status) {
                if(data.data == "success") {
    ...
    $scope.init = function ()
    {
        ...
        if (user != "") {
            ...
            $http({url:"http://localhost:8080/get/u=" + user, method: 'POST'}).then(function(data, status){
                if(data.data == "fail") {
